export default {
    settings: {
      ...
      payment: {
        ...
        redeemPurchaseEnabled: '启用跳转支付',
        redeemPurchaseEnabledHint: '在兑换页面提供购买兑换码的入口，用户购买后可返回本页完成兑换。',
        redeemPurchaseUrl: '兑换码购买链接',
        redeemPurchaseUrlPlaceholder: 'https://example.com/buy-redeem-code',
        redeemPurchaseUrlHint: '必须是完整的 http(s) 链接；启用后用户可从兑换页面跳转。',
        recommendedRedeemPurchaseUrl: '低手续费兑换码购买链接',
        recommendedRedeemPurchaseUrlPlaceholder: 'https://example.com/recommended-redeem-code',
        recommendedRedeemPurchaseUrlHint: '必须是完整的 http(s) 链接；填写后会在兑换页面优先展示为推荐入口。',
